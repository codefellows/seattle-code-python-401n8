...
def main():
    ...
    while True:
        # 1. Printing the options
        console.print("\n1. List Files\n2. Search Files\n3. Move File\n4. Exit")
        choice = Prompt.ask(
            "Choose a task (Enter the number)",
            choices=["1", "2", "3", "4"],
            default="4",
            show_choices=True,  # can turn off with False
            show_default=True,  # can turn off with False
        )

        # 2. Build if/elif/else block
        if choice == "1":
            # Get the user input for a directory
